...
def separated_gp():
	...
	fitted_output_scales, fitted_as, fitted_lengthscales = fitted_params[:, 0], fitted_params[:, 1], fitted_params[:, 2]
...
if __name__ == '__main__':
	...
	plt.subplot(131)
	results_df_output_scales = pd.melt(pd.DataFrame({"Separated GP": fitted_output_scales_sep, "Union GP": fitted_output_scales_union}))
	...
	plt.subplot(132)
	results_df_as = pd.melt(pd.DataFrame({"Separated GP": fitted_as_sep, "Union GP": fitted_as_union}))
	...
	plt.subplot(133)
	results_df_lengthscales = pd.melt(pd.DataFrame({"Separated GP": fitted_lengthscales_sep, "Union GP": fitted_lengthscales_union}))
