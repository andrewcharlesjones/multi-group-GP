...
def separate_GP_experiment():
	...
	for ii in range(n_repeats):
		...
		K_X0X0 = kernel(X0, X0)
		K_X1X1 = kernel(X1, X1)
		Y0 = mvn.rvs(np.zeros(n0), K_X0X0) + np.random.normal(scale=np.sqrt(noise_variance_true), size=n0)
		Y1 = mvn.rvs(np.zeros(n1), K_X1X1) + np.random.normal(scale=np.sqrt(noise_variance_true), size=n1)
		...
		curr_params = [
			...
			np.log(1.), 				  # output variance
			np.log(1.) 					  # length scale
		]
		...
		for jj, alpha in enumerate(alpha_list):
			...
			ll_mggp = mggp.log_marginal_likelihood(curr_params, X, Y, groups=X_groups, group_distances=curr_group_dists)
...
def HGP_experiment():
	...
	length_scale_group0 = 1.
	length_scale_group1 = 1.
